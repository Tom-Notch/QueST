...
def main(cfg):
    ...
    if train_cfg.auto_continue:
        ...
        if "libero" in checkpoint_path and cfg.stage == 2:
            checkpoint_path = checkpoint_path.replace(
                "10", "90"
            )  # since we want to initialize the model from the libero_90 benchmark
    ...
    for epoch in range(start_epoch, train_cfg.n_epochs + 1):
        ...
        for idx, data in enumerate(
            tqdm(train_dataloader, disable=not train_cfg.use_tqdm)
        ):
            ...
            with torch.autograd.set_detect_anomaly(False):
                with torch.autocast(
                    device_type="cuda", dtype=torch.float16, enabled=train_cfg.use_amp
                ):
        ...
        if cfg.rollout.enabled and epoch > 0 and epoch % cfg.rollout.interval == 0:
            rollout_results = env_runner.run(
                model, n_video=cfg.rollout.n_video, do_tqdm=train_cfg.use_tqdm
            )
            print(
