...
class DETRVAE(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "pos_table",
            get_sinusoid_encoding_table(1 + num_queries + n_inputs, hidden_dim),
        )  # [CLS], qpos, a_seq
        ...
        self.additional_pos_embed = nn.Embedding(
            2 + n_decoder_inputs, hidden_dim
        )  # learned position embedding for proprio and latent

    def forward(
        self,
        lowdim_encodings,
        perception_encodings,
        task_emb,
        actions=None,
        is_pad=None,
    ):
        ...
        if is_training:
            ...
            cls_joint_is_pad = torch.full((bs, 1 + self.n_encoder_inputs), False).to(
                lowdim_encodings.device
            )  # False: not a padding
        ...
        try:
            transformer_input = torch.cat(
                [task_emb, latent_input, lowdim_encodings, perception_encodings], dim=0
            )
        except RuntimeError:
            raise ValueError(
                f"something went wrong with the shapes: task_emb: {task_emb.shape}, latent_input: {latent_input.shape}, lowdim_encodings: {lowdim_encodings.shape}, perception_encodings: {perception_encodings.shape}"
            )
...
def build_encoder(
    ...
    encoder_layer = TransformerEncoderLayer(
        d_model, nheads, dim_feedforward, dropout, activation, pre_norm
    )
