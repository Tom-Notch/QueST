...
class Policy(nn.Module, ABC):
    ...
    def __init__(
            ...
            for name, shape in shape_meta["observation"]["rgb"].items():
                ...
                if obs_reduction == "stack" and encoder.out_channels != embed_dim:
                    encoder = nn.Sequential(
                        encoder, nn.ReLU(), nn.Linear(encoder.out_channels, embed_dim)
                    )
        ...
        if shape_meta.task.type == "onehot":
            self.task_encoder = nn.Embedding(
                num_embeddings=shape_meta.task.n_tasks, embedding_dim=embed_dim
            )
        else:
    ...
    def obs_encode(self, data: dict[str, Any], hwc: bool = False, obs_key: str = "obs"):
        ...
        for lowdim_name in self.lowdim_encoders.keys():
            lowdim_encodings.append(
                self.lowdim_encoders[lowdim_name](data[obs_key][lowdim_name])
            )  # add (B, T, H_extra)
