...
class SkillVAE(nn.Module):
    def __init__(
        ...
        assert int(np.log2(downsample_factor)) == np.log2(
            downsample_factor
        ...
        if vq_type == "vq":
            self.vq = VectorQuantize(
                dim=encoder_dim, codebook_dim=codebook_dim, codebook_size=codebook_size
            )
        elif vq_type == "fsq":
    ...
    def quantize(self, z):
        if self.vq_type == "vq":
            ...
            pp = torch.tensor(
                torch.unique(indices).shape[0] / self.vq.codebook_size, device=z.device
            )
        else:
        ...
        pp_sample = torch.tensor(
            np.mean([len(torch.unique(index_seq)) for index_seq in indices])
            / z.shape[1],
            device=z.device,
        )
...
class CausalConv1d(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, dilation, stride, no_pad=False
    ):
...
class DeConv1dBlock(nn.Module):
    ...
    def __init__(
        self, inp_channels, out_channels, kernel_size, stride, n_groups=8, causal=True
    ):
        super().__init__()
        if causal:
            conv = CausalDeConv1d(
                inp_channels, out_channels, kernel_size, dilation=1, stride=stride
            )
        else:
        ...
        if residual:
            if out_channels == inp_channels and stride[0] == 1:
                ...
                self.residual_conv = nn.Conv1d(
                    inp_channels, out_channels, kernel_size=1, stride=sum(stride)
                )
        if pooling_layers:
