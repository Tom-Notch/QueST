...
class LiberoRunner:
    def __init__(
        ...
        descriptions = [
            self.benchmark.get_task(i).language for i in range(self.benchmark.n_tasks)
        ]
    ...
    def run_policy_in_env(self, env_name, policy, render=False):
        ...
        env_fn = lambda: lu.LiberoFrameStack(
            self.env_factory(env_id, self.benchmark), self.frame_stack
        )
        ...
        count = 0
        eval_loop_num = (
            self.rollouts_per_env + self.num_parallel_envs - 1
        ) // self.num_parallel_envs

        while count < eval_loop_num:
            indices = (
                np.arange(count * env_num, (count + 1) * env_num)
                % all_init_states.shape[0]
            )
            init_states_ = all_init_states[indices]
            success, total_reward, episode = self.run_episode(
                env, env_name, policy, init_states_, env_num, render
            )
            count += 1
    ...
    def run_episode(self, env, env_name, policy, init_states_, env_num, render=False):
        ...
        if hasattr(policy, "get_action"):
            ...
            policy = lambda obs, task_id, task_emb: policy_object.get_action(
                obs, task_id, task_emb
            )
