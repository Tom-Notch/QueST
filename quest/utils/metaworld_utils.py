...
classes = {
    ...
    "MT50": {"train": list(_env_names), "test": []},
}
...
def get_env_names(benchmark=None, mode=None):
    ...
    if type(benchmark) is str:
        ...
        env_names = list(
            benchmark.train_classes if mode == "train" else benchmark.test_classes
        )
        env_names.sort()
        return env_names
...
class MetaWorldWrapper(gymnasium.Wrapper):
    ...
    def make_obs(self, obs_gt):
        ...
        for key in self.rgb_outputs:
            obs[key] = image_dict[f"{key[:-4]}2"][
                ::-1
            ]  # since generated dataset at the time had corner key instead of corner2
...
class ML45PRISEBenchmark(object):
    def __init__(self):
        ...
        self.train_classes = {
            name: all_classes[name] for name in classes["ML45_PRISE"]["train"]
        }
        self.test_classes = {
            name: all_classes[name] for name in classes["ML45_PRISE"]["test"]
        }
